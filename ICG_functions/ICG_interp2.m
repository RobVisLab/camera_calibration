function zi = ICG_interp2(varargin)
%ICG_interp2 2-D interpolation (table lookup).
%   ZI = ICG_interp2(X,Y,Z,XI,YI) interpolates to find ZI, the values of the
...
%
%   ZI = ICG_interp2(Z,XI,YI) assumes X=1:N and Y=1:M where [M,N]=SIZE(Z).
%   ZI = ICG_interp2(Z,NTIMES) expands Z by interleaving interpolates between
%   every element, working recursively for NTIMES.  ICG_interp2(Z) is the
%   same as ICG_interp2(Z,1).
%
%   ZI = ICG_interp2(...,'method') specifies alternate methods.  The default
...
%   use the syntax ZI = ICG_interp2(...,'*method').
%
%   ZI = ICG_interp2(...'method',EXTRAPVAL) specificies a method and a scalar 
...
%   monotonic vectors, ICG_interp2 changes them to a plaid internally. 
...
%       zi = ICG_interp2(x,y,z,xi,yi); mesh(xi,yi,zi)
%
...
if (nargin > 1)
  ...
  if method(1)=='*', % Direct call bypass.
     ...
     elseif method(2)=='s', % spline interpolation
         ...
         error('MATLAB:ICG_interp2:InvalidMethod',...
...
elseif narg==3, % ICG_interp2(z,xi,yi)
  [nrows,ncols] = size(varargin{1});
  x = 1:ncols; y = (1:nrows);
  [msg,x,y,z,xi,yi] = xyzchk(x,y,varargin{1:3});

elseif narg==4,
  error('MATLAB:ICG_interp2:nargin','Wrong number of input arguments.');
...
%   error('MATLAB:ICG_interp2:meshgrid',...
%         sprintf(['X and Y must be matrices produced by MESHGRID. Use' ...
%                  ' GRIDDATA instead \nof ICG_interp2 for scattered data.']));
% end
...
if ~bypass,
  ...
  if (xdiff > eps*max(abs(xx))) || (ydiff > eps*max(abs(yy))),
    ...
    if any(dx<=0) || any(dy<=0), 
      error('MATLAB:ICG_interp2:XorYNotMonotonic',...
...
elseif nargin==5,
  error('MATLAB:ICG_interp2:linear:nargin','Wrong number of input arguments.');
...
elseif nargin==6, % linear(x,y,z,s,t), X and Y specified.
  ...
  if any([mx my] ~= [ncols nrows]) && ...
    ...
    error('MATLAB:ICG_interp2:linear:XYZLengthMismatch',...
          'The lengths of the X and Y vectors must match Z.');
  end
  if any([nrows ncols]<[2 2])
    error('MATLAB:ICG_interp2:linear:sizeZ','Z must be at least 2-by-2.'); 
  end
...
if any([nrows ncols]<[2 2])
  error('MATLAB:ICG_interp2:linear:sizeZsq','Z must be at least 2-by-2.'); 
end
if ~isequal(size(s),size(t)),
  error('MATLAB:ICG_interp2:linear:XIandYISizeMismatch',...
...
elseif nargin==5,
  error('MATLAB:ICG_interp2:cubic:nargin','Wrong number of input arguments.');
...
elseif nargin==6, % cubic(x,y,z,s,t), X and Y specified.
  ...
  if any([mx my] ~= [ncols nrows]) && ...
     ~isequal(size(arg1),size(arg2),size(arg3))
    error('MATLAB:ICG_interp2:cubic:XYZLengthMismatch',...
          'The lengths of the X and Y vectors must match Z.');
  end
  if any([nrows ncols]<[3 3])
    error('MATLAB:ICG_interp2:cubic:sizeZ','Z must be at least 3-by-3.'); 
  end
  s = 1 + (arg4-arg1(1))/(arg1(mx)-arg1(1))*(ncols-1);
  t = 1 + (arg5-arg2(1))/(arg2(my)-arg2(1))*(nrows-1);
...
if any([nrows ncols]<[3 3]) 
  error('MATLAB:ICG_interp2:cubic:sizeZsq','Z must be at least 3-by-3.'); 
end
if ~isequal(size(s),size(t)), 
  error('MATLAB:ICG_interp2:cubic:XIandYISizeMismatch','XI and YI must be the same size.');
end
...
%   NEAREST(Z,1).
%
%   See also ICG_interp2, LINEAR, CUBIC.
...
elseif nargin==5,
  error('MATLAB:ICG_interp2:nearest:nargin','Wrong number of input arguments.');
...
elseif nargin==6, % nearest(x,y,z,u,v), X and Y specified.
  ...
  if any([mx my] ~= [ncols nrows]) & (size(arg1)~=size(arg3) |   ...
    size(arg2)~=size(arg3)), 
    error('MATLAB:ICG_interp2:nearest:XYZLengthMismatch',...
          'The lengths of the X and Y vectors must match Z.');
  end
...
if size(u)~=size(v) 
  error('MATLAB:ICG_interp2:nearest:XIandYISizeMismatch',...
        'XI and YI must be the same size.'); 
end
