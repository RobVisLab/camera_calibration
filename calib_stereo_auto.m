...
left_active_images = active_images;
load Calib_Results_right;
right_active_images = active_images;
load Calib_Results_left;
overall_active_images = (left_active_images+right_active_images) == 2;
...
active_images_left = overall_active_images;
...
active_images_right = overall_active_images;
...
for iter = 1:12;
    ...
    for kk = 1:n_ima,
        ...
        if overall_active_images(kk),
            ...
            eval(['Xckkl = X_left_' num2str(kk) ';']);
            eval(['Xckkr = X_right_' num2str(kk) ';']);
            [C,ia,ib]=intersect(Xckkl',Xckkr','rows');
            Xckk = C';
            eval(['omckk = omc_left_' num2str(kk) ';']);
            eval(['Tckk = Tc_left_' num2str(kk) ';']);
            
            eval(['xlkk = x_left_' num2str(kk) '(:,ia);']);
            eval(['xrkk = x_right_' num2str(kk) '(:,ib);']);
    ...
    ind_Jac = find([est_fc_left & [1;est_aspect_ratio_left];center_optim_left*ones(2,1);est_alpha_left;est_dist_left;est_fc_right & [1;est_aspect_ratio_right];center_optim_right*ones(2,1);est_alpha_right;est_dist_right;ones(6,1);reshape(ones(6,1)*overall_active_images,6*n_ima,1)]);
    
    ind_active = find(overall_active_images);
